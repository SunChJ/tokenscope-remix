...
- Context health: median/peak context-window pressure, ≥80% warnings, compaction count, and Codex reasoning-token share
...
- Tool classification: MCP calls are attributed from direct `mcp__<server>__*` names, Codex tool-search mappings, or Codex App custom calls, then checked against the owning agent's config; Claude Skill tool/slash-command calls and Codex `SKILL.md` reads are matched against that agent's user/project skill directories; everything else is ignored
